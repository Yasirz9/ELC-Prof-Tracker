// Server-only helpers and re-exports for proofs.functions.ts
// Importing this file from client code is blocked by the *.server.* pattern.
...
export async function requireSuperAdminUserId(accessToken: string): Promise<string> {
  if (!accessToken) throw new Error("Unauthorized");
  const { data, error } = await supabaseAdmin.auth.getUser(accessToken);
  if (error || !data.user) throw new Error("Unauthorized");
  const { data: row } = await supabaseAdmin
    ...
    .eq("role", "super_admin")
    .maybeSingle();
  if (!row) throw new Error("Forbidden: super admin only.");
  return data.user.id;
}
